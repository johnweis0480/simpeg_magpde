...
#A function to transform to the shared constraints, here it is the amplitude of the 3D model
def transform(vec):
    return np.linalg.norm((vec).reshape(
            (-1, 3), order="F"
        ),
        axis=1,
    )

#A function to project shared constraints back to the model-space
def project(result):
...
opt_shared = optimization.ProjectedGNCG_shared_constraint(
    transform=transform,project_back=project, upper_shared = .025,maxIter=20, lower=-10, upper=10.0, maxIterLS=20, maxIterCG=20, tolCG=1e-4
)


invProb = inverse_problem.BaseInvProblem(dmis, reg, opt_shared)
...
mrec_constraints = inv.run(m0)

print('Maximum amplitude: '+str(np.max(transform(mrec_constraints)))[:5])
...
for ii, (title, mvec) in enumerate(
    [("True model", model), ("Smooth model", invProb.l2model), ("Sparse model", mrec_constraints)]
):
